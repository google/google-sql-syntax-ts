...
export class GoogleSqlDefinition implements SqlDefinition {
  ...
  private readonly FUNCTIONS_WITH_INFO: FunctionDescription[] = [
    ...
    },
    {
      name: 'TANH',
      args: 'numeric_expr',
      description: 'Computes hyperbolic tangent of <numeric_expr>. Does ' +
          'not fail.',
    },
    {
      name: 'TIMESTAMP',
  ...
  /** Functions. */
  readonly FUNCTIONS: string[] = this.mergedSqlFunctions().map(
      (elem) => {
        return elem.name;
      },
  );
  ...
  private readonly TVF_FUNCTIONS = this.TVF_FUNCTIONS_WITH_INFO.map(
      (elem) => {
        return elem.name;
      },
  );
...
/** The exported languaged definition for google sql. */
export function getGoogleSqlLanguageDefinition(docsUrls: DocsUrls):
    GoogleSqlLanguageDefinition {
  const googleSqlDefinition = new GoogleSqlDefinition(docsUrls);
  return {
    ignoreCase: true, defaultToken: '', tokenPostfix: '.googleSQL',

        // Merge all keywords into one list
        keywords: googleSqlDefinition.FUNCTIONS
            .concat(
        ...
        digits: /\d+(_+\d+)*/, octaldigits: /[0-7]+(_+[0-7]+)*/,
