...
class QueryFormatter {
    constructor(sqlDefinition) {
        this.sqlDefinition = sqlDefinition;
        this.stringsAndComments = [];
    }
    /** Formats the given query string. */
    formatQuery(query) {
        query = query.replace(this.getStringsAndCommentsRegEx(), (str) => this.addToStringsAndComments(str));
        ...
        query = query.replace(new RegExp(/create\s+or replace /gi), (v) => v.toUpperCase().replace(/\s+/g, ' '));
        ...
        for (const connectClause of this.sqlDefinition.getConnectedClauseWords()) {
            ...
            const regex = new RegExp('(' + regExpEscape(connectClause.first) + ')(\\s|\\n)+(' +
                regExpEscape(connectClause.second) + ')', 'g');
    ...
    indent(query) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const lineWithoutComment = trimmed.replace(this.getStringsAndCommentsRegEx(), '');
     ...
     */
    replaceStringsAndComments(query, singleLineComments) {
        ...
        for (const [index, str] of this.stringsAndComments.entries()) {
            if (this.isSingleLineComment(str) === singleLineComments) {
                ...
                // Unlikely.
                query = query.replace(new RegExp(regExpEscape(`${UNIQUE_PREFIX}${index + 1}${UNIQUE_SUFFIX}`), 'g'), () => 
    ...
    getEscapedWordsRegex() {
        return (regExpEscape(this.sqlDefinition.getBracketQuoteStart()) + '(.*?)' +
